...
from app.schemas.detection import MAX_BATCH_SCAN_ITEMS, ScanRequest, ScanResponse
...
def scan_batch(
    payloads: Annotated[List[ScanRequest], Field(max_length=MAX_BATCH_SCAN_ITEMS)],
    request: Request,
    db: Session = Depends(get_db),
    current_user: Administrator = Depends(get_current_user),
):
